...
async def accept_order_cb(callback: CallbackQuery, session: AsyncSession, lang: str):
    order_id = int(callback.data.split(":")[1])
    staff_user = await get_user_by_telegram_id(session, callback.from_user.id)

    success = await accept_order(session, order_id, staff_user.id)
...
async def reject_order_cb(callback: CallbackQuery, session: AsyncSession, lang: str):
    order_id = int(callback.data.split(":")[1])
